...
- **`Advanced-Uninstaller.ps1`**: A robust, universal uninstallation framework designed to rip out stubborn applications by deeply scanning the registry (`Uninstall` and `Installer\Products` hives), killing blocking processes, and dynamically executing hidden uninstaller binaries.
- **`PSADT-GoogleChrome.ps1`**: A highly robust, enterprise-grade deployment of Google Chrome using PSADT. Features custom `master_preferences` injection, silent edge-case handling, and deeply enforced user-level configuration blocking.
